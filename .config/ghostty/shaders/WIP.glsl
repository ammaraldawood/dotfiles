...
const vec4 TRAIL_COLOR_ACCENT = vec4(1.0, 0.549, 0.0, 1.0);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    float mod = .007;
    //trailblaze
    vec4 trail = mix(TRAIL_COLOR_ACCENT, fragColor, 1. - smoothstep(0.02, sdfTrail + mod, 0.01));
    trail = mix(TRAIL_COLOR, trail, 1. - smoothstep(0.01, sdfTrail + mod, 0.011));
    trail = mix(trail, TRAIL_COLOR, step(sdfTrail + mod, 0.));
    //cursorblaze
    trail = mix(TRAIL_COLOR_ACCENT, trail, 1. - smoothstep(0.01, sdfCurrentCursor + .05, 0.01));
    trail = mix(TRAIL_COLOR, trail, 1. - smoothstep(0.02, sdfCurrentCursor + .01, 0.01));
    fragColor = mix(trail, fragColor, 1. - smoothstep(0.001, sdfCurrentCursor, easedProgress * lineLength));
}
